...
func UpdateAttestationStreaks() (updatedToLastFinalizedEpoch bool, err error) {
	...
	day := int(startEpoch / 1080)

	if int(endEpoch/1080) > day {
		endEpoch = (day+1)*1080 - 1
	}
	...
	if startEpoch == endEpoch {
		...
		boundingsQry = `boundings as (select validatorindex, $2+1 as epoch, status from attestation_assignments_p where week = $2/7560 and epoch = $2),`
	} else {
		// use validator_stats table to limit search-space
		nomissesQry := `select validatorindex, $2+1 as epoch, 1 as status from validator_stats where day = $1/1080 and (missed_attestations = 0 or missed_attestations is null) and validatorindex != 2147483647`
		if !statsExist {
			// if the validator_stats table has no entry for this day we find validators with only misses or no misses
			nomissesQry = `select validatorindex, $2+1 as epoch, status from attestation_assignments_p where week = $1/7560 and epoch >= $1 and epoch <= $2 group by validatorindex, status having count(*) = $2-$1+1`
		}
		boundingsQry = fmt.Sprintf(`
			...
			aa as (
				...
				where week = $1/7560 and epoch >= $1 and epoch <= $2 and validatorindex not in (select validatorindex from nomisses)
			),
			-- find boundings
			boundings as (
